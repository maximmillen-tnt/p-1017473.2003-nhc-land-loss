...
the evacuated and inundated area on every property it reached, each with the
depth of the material involved.
...
polygons may overlap and ground buried twice is buried once.
...
def describe_damage(damaged, insured, landslides):
    ...
    print(f"Properties reached: {len(damaged):,} of {len(insured):,}")
    if damaged.empty:
        return
    for kind in ("evacuated", "inundated"):
        ...
        hit = area > 0
        print(
            f"  {kind}: {int(hit.sum()):,} properties, {area.sum():,.0f} m2 total, "
            f"median {area[hit].median():,.1f} m2 where present"
        )
    over = check_within_insured_area(damaged, insured)
    if over.empty:
        print("  no property carries more damaged ground than it has insured land")
    else:
        print(f"  {len(over):,} properties carry more damage than insured land")


def main(*, pilot, realisation_ids):
    """Write the landslide damaged area per property, per realisation."""
    ...
    for realisation_id in realisation_ids:
        ...
        damaged = damaged_area_per_property(insured, landslides)
        damaged.insert(0, "realisation_id", realisation_id)
