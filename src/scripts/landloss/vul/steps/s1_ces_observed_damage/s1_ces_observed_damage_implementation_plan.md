...
- Move the figure script into this folder. It sits in
  `src/scripts/landloss/vul/report/` because that is where the project lead asked
  for it, which predates the step folder convention in the `adding-steps-scripts`
  skill; the skill would put it here beside the step it belongs to.
