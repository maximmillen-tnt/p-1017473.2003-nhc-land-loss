...
- `classify_landform` assigns hill and flat only. The third class,
  ...
  `landloss.exposure.landform.assign_elevated_flat` from a terrain attribute,
...
- A flat address standing more than
  ...
  `landloss.exposure.landform.assign_elevated_flat`. A hill address is never
...
- Within a landform class, value is spread by the continuous terrain modifier
  `landloss.exposure.land_value.terrain_modifier`: slope and topographic
...
- The two terrain attributes themselves are shown in the figure produced by
  ...
  `report/exposure/land-value/fig/`: slope on a sequential ramp pinned at zero,
