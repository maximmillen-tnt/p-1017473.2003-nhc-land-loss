...
- The database is written as `observed_damage_db.parquet` under the vul module's
  ...
  further layers.
- Settled land damage against LSN is plotted by `fig_land_damage_v_lsn.py` in
  `src/scripts/landloss/vul/liquefaction/land/report/` as a six panel figure —
  one panel holding every property and five splitting them by observed damage
  category — written to `report/vul/liquefaction/land/fig/`. Damage is clipped to
